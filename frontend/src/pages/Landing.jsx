import { ArrowRight, Camera, Check, Languages, Mic2, Play, Quote, ScanLine, Sparkles, Store, WandSparkles } from "lucide-react";
import { Link } from "react-router-dom";
import { motion } from "framer-motion";
import { SAMPLE_BUSINESSES, SAMPLE_PRODUCTS } from "@/lib/sampleBusinesses";

const steps = [
  { n: "01", icon: Camera, title: "Show the product", copy: "Take one honest phone photo. No lightbox, catalogue template or technical setup." },
      ...
      <main>
        <section className="relative mx-auto grid min-h-[calc(100vh-80px)] max-w-[1440px] items-center gap-12 px-5 pb-16 pt-8 md:px-10 lg:grid-cols-[1.02fr_.98fr] lg:px-14 lg:pb-20 lg:pt-6">
          <div className="relative z-10">
            ...
            <motion.p initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} transition={{delay:.16}} className="mt-8 max-w-xl text-lg leading-relaxed text-[#54564F] md:text-xl">
              Angadi helps offline shopkeepers become online sellers using only a product photo and their voice.
            </motion.p>
            <motion.div initial={{opacity:0,y:16}} animate={{opacity:1,y:0}} transition={{delay:.24}} className="mt-8 flex flex-col gap-3 sm:flex-row">
              <Link to="/products/new" className="group inline-flex items-center justify-center gap-3 rounded-full bg-[#FF5C35] px-7 py-4 text-sm font-bold text-white shadow-[0_16px_40px_-18px_rgba(255,92,53,.8)] transition hover:-translate-y-0.5 hover:bg-[#E94B27]">
                Digitise your first product <ArrowRight className="h-4 w-4 transition group-hover:translate-x-1" />
              </Link>
              <Link to="/store/yuva" className="inline-flex items-center justify-center gap-3 rounded-full border border-[#181A17]/20 bg-white/50 px-7 py-4 text-sm font-bold backdrop-blur transition hover:bg-white">
                <Play className="h-4 w-4 fill-current" /> See the Yuva store
              </Link>
            </motion.div>
            <motion.div initial={{opacity:0}} animate={{opacity:1}} transition={{delay:.35}} className="mt-10 flex flex-wrap gap-x-7 gap-y-3 text-xs font-semibold uppercase tracking-[.12em] text-[#696B64]">
              <span className="flex items-center gap-2"><Check className="h-4 w-4 text-[#FF5C35]" /> One photo</span>
        ...
        <div className="overflow-hidden border-y border-[#181A17] bg-[#181A17] py-4 text-[#F3EFE5]">
          <div className="landing-marquee flex w-max items-center gap-8 whitespace-nowrap text-sm font-bold uppercase tracking-[.2em]">
            {[0,1].map(group => <div key={group} className="flex items-center gap-8"><span>Photo lo</span><span className="text-[#D9F15B]">✦</span><span>Bolo</span><span className="text-[#FF5C35]">✦</span><span>Online becho</span><span className="text-[#D9F15B]">✦</span><span>Your language</span><span className="text-[#FF5C35]">✦</span><span>Your store</span><span className="text-[#D9F15B]">✦</span></div>)}
          </div>
        </div>
          ...
          <div className="mx-auto grid max-w-[1320px] gap-14 lg:grid-cols-[.8fr_1.2fr]">
            ...
            <div>
              ...
              <div className="mt-10 grid grid-cols-3 gap-3 border-t border-white/15 pt-8">
                <div><div className="text-3xl font-semibold text-[#D9F15B] md:text-5xl">93%</div><div className="mt-2 text-xs text-white/45">of Indian retail sales remain offline</div></div>
                <div><div className="text-3xl font-semibold text-[#FF5C35] md:text-5xl">1</div><div className="mt-2 text-xs text-white/45">photo replaces a long product form</div></div>
              ...
              <a href="https://www.deloitte.com/in/en/about/press-room/india-s-us-1-06-trillion-retail-sector-is-set-to-reach-1-93-trillion-by-2030.html" target="_blank" rel="noreferrer" className="mt-5 inline-block text-[10px] text-white/30 underline decoration-white/20 underline-offset-4">Retail estimate: Deloitte–FICCI, 2024</a>
            </div>
          </div>
        </section>
